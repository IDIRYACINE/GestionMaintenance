...
public class ProductQuery extends IProductQuery {
    ...
    @Override
    public ResultSet LoadProductsByInBarcodes(String barcodes, boolean notIn) throws SQLException {
        String query = "SELECT * FROM " + ETables.Inventory + " WHERE " + EInventoryAttributes.ArticleCode;
        if (notIn)
            query += " NOT IN (" + barcodes + ")";
        else
            query += " IN (" + barcodes + ")";

        query += " ORDER BY " + EInventoryAttributes.ArticleId;
    ...
    @Override
    public ResultSet LoadScannedInventory() throws SQLException {
        String query = "SELECT * FROM " + ETables.ScannedArticles;
